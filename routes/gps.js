...
router.post('/position', async (req, res) => {
  try {
    ...
    if (busQuery.empty) return res.status(404).json({ error: true, message: 'No se encontró bus con ese GPS tracker' });

    const busDoc = busQuery.docs[0];
    const busId = busDoc.id;

    const posicionData = {
      bus_id: busId, device_id,
    ...
    if (!alumnosQuery.empty) await checkProximityAlerts(busId, latitude, longitude, alumnosQuery.docs);

    res.json({ success: true, message: 'Posición GPS guardada exitosamente', data: { bus_id: busId, latitude: parseFloat(latitude), longitude: parseFloat(longitude), students_count: alumnosQuery.size } });
...
router.get('/bus/:busId/current', async (req, res) => {
  try {
    ...
    res.json({ success: true, data: { bus_id: busId, placa: busData.placa, ubicacion: busData.ubicacion_actual, ultima_actualizacion: busData.ultima_actualizacion } });
  } catch (error) {
...
router.post('/update-position/:busId', async (req, res) => {
  try {
    ...
    const ubicacion = { latitude: parseFloat(latitude), longitude: parseFloat(longitude), speed: speed ? parseFloat(speed) : 0, timestamp: new Date().toISOString() };
...
router.post('/traccar-webhook', async (req, res) => {
  try {
    ...
    if (!id || !lat || !lon) return res.status(400).json({ error: true, message: 'Datos incompletos en webhook' });

    const deviceId = id;
    const busesSnapshot = await db.collection('buses').where('gps_imei', '==', deviceId).limit(1).get();
    if (busesSnapshot.empty) return res.status(404).json({ error: true, message: `Bus no encontrado para dispositivo ${deviceId}` });
    ...
    const ubicacion = {
      ...
      source: 'traccar', deviceId
    };

    await db.collection('buses').doc(busId).update({ ubicacion_actual: ubicacion, ultima_actualizacion: new Date().toISOString(), updatedAt: new Date().toISOString() });
    await db.collection('gps_positions').add({ bus_id: busId, placa: bus.placa, latitude: ubicacion.latitude, longitude: ubicacion.longitude, speed: ubicacion.speed, altitude: ubicacion.altitude, course: ubicacion.course, source: 'traccar', device_id: deviceId, timestamp: ubicacion.timestamp, createdAt: new Date().toISOString() });

    const alumnosQuery = await db.collection('alumnos').where('bus_id', '==', busId).get();
    if (!alumnosQuery.empty) await checkProximityAlerts(busId, ubicacion.latitude, ubicacion.longitude, alumnosQuery.docs);
 ...
 * GET /api/gps/history
 */
router.get('/history', async (req, res, next) => { next(); }, verifyToken, async (req, res, next) => { next(); }, checkCamionAccess, async (req, res) => {
  try {
...
async function checkProximityAlerts(busId, busLat, busLng, alumnosDocs) {
  ...
  try {
    ...
    // Cooldown configurable (default 10 min)
    let cooldownMinutos = 10;
    ...
    if (colegioId) {
      ...
      if (colegioDoc.exists) {
        ...
        cooldownMinutos = colegioData.configuracion_notificaciones?.cooldown_minutos || 10;

        // Verificar horario
        const horarios = colegioData.horarios_mapa;
    ...
    for (const alumnoDoc of alumnosDocs) {
      ...
      for (const padre of padres) {
        ...
        // Verificar cooldown configurable
        ...
        if (cooldownDoc.exists) {
          ...
          if (minutesPassed < cooldownMinutos) {
            console.log(`⏱️ Cooldown activo para ${alumno.nombre}, faltan ${Math.round(cooldownMinutos - minutesPassed)} min`);
            continue;
          }
        }
...
function estaEnHorarioActivo(horarios) {
  ...
  const enIda = horarios.ida?.activo && horaActual >= timeToMin(horarios.ida.inicio) && horaActual <= timeToMin(horarios.ida.fin);
  const enRegreso = horarios.regreso?.activo && horaActual >= timeToMin(horarios.regreso.inicio) && horaActual <= timeToMin(horarios.regreso.fin);
